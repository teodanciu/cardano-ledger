...
data CollInfo era = CollInfo
  { bal :: Coin, -- Balance of all the collateral inputs
    ret :: Coin, -- Coin amount of the collateral return output
    delset :: Set (TxIn (Crypto era)), -- The set of inputs to delete from the UTxO
    addmap :: Map (TxIn (Crypto era)) (Core.TxOut era) -- Things to Add to the UTxO
  }
...
collInfo :: Reflect era => Proof era -> Int -> Model era -> CollInfo era -> TxBodyField era -> CollInfo era
collInfo proof count model info field = case field of
  ...
  CollateralReturn (SJust txout) ->
    case Map.lookup count (mIndex model) of
      Nothing -> error "Output not found"
      Just (TxId hash) -> info {ret = getTxOutCoin proof txout, addmap = newstuff}
        where
          newstuff = additions hash [txout]
  Collateral inputs ->
    info
      { delset = inputs,
        bal = txInBalance inputs (mUTxO model)
      }
  _ -> info
...
updateInfo info m =
  m
    { mUTxO = Map.union (addmap info) (Map.withoutKeys (mUTxO m) (delset info)),
      mfees = (mfees m) <+> (bal info) <-> (ret info)
    }
