...
-- pcTxOut,
import Cardano.Ledger.Alonzo.Tx (IsValid (..), ScriptPurpose (Certifying, Spending))
...
lookupByKeyM ::
  (Ord k, Show k) => String -> k -> (GenState era -> Map.Map k v) -> GenRS era v
...
genFreshTxIn tries = do
  ...
  NonEmpty ins <- lift $ resize 30 arbitrary
  case filter (not . (`Map.member` entrysInUse)) ins of
    [] -> genFreshTxIn (tries - 1)
    freshTxIns -> pure freshTxIns

-- ====================================================================
-- Generating UTxO and associated Inputs (Set (TxIn era))

-- | Generate a somewhat arbitrary MUtxo,  Ocasionally add a bit of
...
genUTxO = do
  ...
  pairs <- sequence (map (\x -> (x,) <$> genOut) ins) -- (filter (not . (`Map.member` entrysInUse)) ins))
  percent <- gets getOldUtxoPercent
  maybepair <- frequencyT [(percent, getUtxoElem), (100 - percent, pure Nothing)] -- Choose a pair from the oldUTxO
  pure (Map.fromList (maybeCons maybepair pairs), maybepair)
  where
    -- Note: Never add an old pair unless there are at least 1 new pairs
...
genSpendReferenceInputs newUTxO = do
  ...
  numInputs <- lift $ choose (1, min (length pairs) maxinputs)
...
genDCerts = do
  ...
  -- DPState {dpsDState = DState {_unified}} <- gsDPState <$> ask
