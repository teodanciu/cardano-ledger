...
getNewPoolTest = do
  ...
  pure (\k -> maybe True (const False) (Map.lookup k poolparams))
...
genRewards = do
  ...
  old <- gets (Map.keysSet . gsInitialRewards) -- we need a fresh credential, one that was not previously
  prev <- gets gsRegKey -- generated here, or one that arose from RegKey (i.e. prev)
  ...
  -- always generate a New Reward.
  -- Map.fromList <$> replicateM n ((,) <$> frequencyT [ (1, KeyHashObj <$> genKeyHash),(35, ScriptHashObj <$> genScript reify Rewrd)] <*> lift genRewardVal)
...
-- | Generate a 'n' fresh credentials (ones not in the set 'old'). We get 'trys' chances,
--   if it doesn't work in 'trys' attempts then quit with an error. Better to raise an error
...
genFreshCredentials 0 _trys _tag _old ans = pure ans
genFreshCredentials _n 0 _tag _old _ans = error ("Ran out of trys in genFreshCredentials.")
genFreshCredentials n trys tag old ans = do
  c <- genFreshCredential trys tag old
  genFreshCredentials (n - 1) trys tag (Set.insert c old) (c : ans)
...
genFreshCredential ::
  ...
  Set (Credential kr (Crypto era)) ->
  GenRS era (Credential kr (Crypto era))
genFreshCredential 0 _tag _old = error ("Ran out of trys in genFreshCredential.")
genFreshCredential trys tag old = do
  c <- genCredential tag
  if Set.member c old
    then genFreshCredential (trys - 1) tag old
    else pure c
...
initialLedgerState gstate = LedgerState utxostate dpstate
  where
    ...
    -- In a wellformed LedgerState the the deposited equals the obligation
    deposited =
      obligation'
        reify
