...
babbageTxInfoV2 timeRange tx utxo = do
  inputs <- mapM (txInfoInV2 utxo) (Set.toList ((trace "CHECKING INPUTS" txBody) ^. inputsTxBodyL))
  refInputs <- mapM (txInfoInV2 utxo) (Set.toList ((trace "CHECKING REFS" txBody) ^. referenceInputsTxBodyL))
  outputs <-
    zipWithM
